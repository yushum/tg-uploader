...
def archive_video_file(source_filepath: str, archive_reference_filepath: str | None = None) -> str:
    """Archive a complete video, using a separate path as its archive name when needed."""
    source = Path(source_filepath)
    archive_path = get_archive_path(archive_reference_filepath or source_filepath)
...
async def upload_file(client: TelegramClient, filepath: str, conn: sqlite3.Connection, semaphore: asyncio.Semaphore, fail_counts: dict):
    ...
    async with semaphore:
        ...
        # Archive the complete, directly playable MP4 before Telegram-specific splitting.
        # Uploader-generated split parts are marked explicitly and must not be archived again.
        if initial_status != 'SPLIT_PART_PENDING' and should_archive(filepath):
            archive_reference_path = filepath
            if is_temp_mp4:
                archive_reference_path = str(Path(filepath).with_suffix('.mp4'))
            try:
                ...
                logger.error(
                    f"Failed to archive complete video for {filepath}; "
                    f"upload cancelled to protect the recording: {archive_error}"
                )
                update_upload_status(conn, filepath, 'FAILED')
                fail_counts[filepath] = fail_counts.get(filepath, 0) + 1
                return
