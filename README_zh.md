...
播放器支持跨分片连续播放、精确进度、缓冲与错误恢复、前后 10 秒、0.5～3 倍速、音量记忆、画中画、全屏切换、桌面快捷键、移动端双击快进/快退和长按加速。媒体 Range 会按块缓存在 `./cache`，减少拖动进度时对 Telegram 的重复读取。默认最多使用 2GB，缓存块 24 小时未访问后自动清理，可通过 `MEDIA_CACHE_MAX_MB` 和 `MEDIA_CACHE_MAX_AGE` 调整。
