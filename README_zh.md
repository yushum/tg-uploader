...
* **内存节流防御 (OOM Defense) 与抗频控**：集成 `jemalloc` 内存分配器杜绝长时间运行下的内存碎片，配合精准的 Semaphore 并发锁与滑动内存映射读取。面对 40GB 的巨型视频也只占用极其微小的内存。自动接管并处理 Telegram 的各种请求速率超限 (`FloodWaitError`)。
* **阅后即焚全托管**：在确认文件 100% 成功上传到频道并写入 SQLite 本地记录后，系统将自动将物理硬盘上的原始视频彻底粉碎删除，并递归清理生成的空文件夹，确保你的硬盘永远干净如新。
* **选择性本地归档**：可按主播名或路径关键词保留完整原片，便于之后切片。同盘时优先用硬链接，并可按保留天数自动清理。
...
`docker compose up -d` 会在上传器容器内同时启动录播网站，并仅监听宿主机的 `127.0.0.1:31527`，可直接接入 Cloudflare Tunnel。网站从现有数据库按主播和日期整理录像，通过 Telegram 分段读取视频，并使用有容量和有效期限制的本地分块缓存改善拖动体验。多个录像分片会在同一个播放器中连续播放。

首次启动时，网站会从现有 `uploader.session` 创建独立的 `web.session`，避免播放影响上传连接。页面默认跟随设备的浅色或深色外观，也可在右上角手动选择；深色主题使用纯黑背景。

播放器支持跨分片连续播放、精确进度、缓冲与错误恢复、前后 10 秒、0.5～3 倍速、音量记忆、90 度旋转、画中画、全屏切换、桌面快捷键、移动端双击快进/快退和长按加速。媒体 Range 会按块缓存在 `./cache`，减少拖动进度时对 Telegram 的重复读取。默认最多使用 2GB，缓存块 24 小时未访问后自动清理，可通过 `MEDIA_CACHE_MAX_MB` 和 `MEDIA_CACHE_MAX_AGE` 调整。
