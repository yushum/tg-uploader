...
# Modified for TG Uploader to support parallel connections
import asyncio
import hashlib
import inspect
import logging
...
class UploadSender:
    ...
    async def _next(self, data: bytes) -> None:
        ...
        # Retry loop inside the worker sender
        retries = 0
        while True:
            try:
                ...
                log.warning(f"FastTelethon Upload Worker hit FloodWait! Sleeping for {wait_time}s...")
...
class ParallelTransferrer:
    ...
    async def init_upload(self, file_id: int, file_size: int, part_size_kb: Optional[float] = 512,
        ...
        part_size = int(part_size_kb * 1024) if part_size_kb else utils.get_appropriated_part_size(file_size) * 1024
...
def stream_file(file_to_stream: BinaryIO, chunk_size=512*1024):
    ...
    import mmap
    file_size = os.fstat(file_to_stream.fileno()).st_size
    if file_size == 0:
        return
    # Map the entire file to memory (read-only)
    with mmap.mmap(file_to_stream.fileno(), length=0, access=mmap.ACCESS_READ) as mm:
        offset = 0
        while offset < file_size:
            # Memory slicing is extremely fast and avoids blocking the asyncio event loop
